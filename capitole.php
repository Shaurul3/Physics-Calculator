...
<body>
    ...
    <div class="container">
        ...
        <div class="tab-content" id="myTabContent" style="background-color:white">
            <?php
            $sql = "SELECT R.NumeRamura, C.NumeCapitol, C.ClasaStudiu
        FROM Capitol C
        INNER JOIN Ramura R ON C.RamuraID = R.RamuraID";
            ...
            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                ...
                $clasaStudiu = $row['ClasaStudiu'];
                $numeRamura = $row['NumeRamura'];

                if (!isset($capitole[$numeRamura])) {
                    $capitole[$numeRamura] = array();
                }

                $capitole[$numeRamura][] = array(
                    "numeCapitol" => $numeCapitol,
                    "clasaStudiu" => $clasaStudiu
                );
            }

            foreach ($capitole as $numeRamura => $capitoleRamura) {
                $idTab = strtolower(str_replace([' ', '(', ')'], '_', $numeRamura));
                echo '<div class="tab-pane fade" id="' . $idTab . '" role="tabpanel" aria-labelledby="' . $idTab . '-tab">';
                foreach ($capitoleRamura as $capitol) {
                    $numeCapitol = $capitol['numeCapitol'];
                    $clasaStudiu = $capitol['clasaStudiu'];
                    echo "<h4>$numeCapitol - $clasaStudiu</h4>";
                }
                echo "</div>";
            }
